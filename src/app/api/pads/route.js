...
export async function POST(request) {
  ...
  if (!rate.allowed) return jsonError("Too many requests. Try again soon.", 429);
  ...
  if (isReservedSlug(slug)) return jsonError("That pad ID is reserved. Choose a different one.", 400);
  const result = await createPad({ slug, password: password || "" });
  if (result.error) return jsonError(result.error, 409);

  return NextResponse.json({ slug: result.pad.slug, passwordProtected: Boolean(result.pad.passwordHash) });
}
